...
void mem_init(void *base, size_t length) {
    ...
    while ((1UL << (max_order + 1)) <= total_len && max_order + 1 < BUDDY_MAX_ORDER) //1UL << max_order+1 es equivalente a 2^(max_order+1)
        max_order++;
...
void *mem_alloc(size_t size) {
    ...
    while (o > order) { //desde el actual al minimo
...
void mem_free(void *ptr) {
    ...
    while (order < max_order) {
        ...
        if (!n) break; // buddy ocupado (no mergeo)
